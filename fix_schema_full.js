...
// Fix Date to number error in router
// Error: Type 'Date' is not assignable to type 'number'.
router = router.replace(/lastEnrichedAt: new Date\(\)/g, 'lastEnrichedAt: new Date()'); // mode:timestamp should fix this

let db = fs.readFileSync('server/db.ts', 'utf-8');
db = db.replace(/eq\(apiSettings\.isActive, true\)/g, 'eq(apiSettings.isActive, true)'); // mode:boolean should fix this as well.
